    ...
    if count != 0:
        ...
        try:
            ...
            important_info_content[
                0] = "以下是'需要注意到的重要信息'，你应当根据当前场景有选择性地参考他们，而不是全部参考他们。分数越高的信息越重要。\n\n" + \
            ...
            if enable_likeability[0] == 1:
                change_likeability_task = asyncio.create_task(
                    change_likeability(merge_result, result["content"], current_likeability[0], memories, 3,
                                       user_name[0]))
            ...
            if len(memories) > 3:
                ...
                t1 = asyncio.create_task(
                    get_translation_part(memories[-3:], temp_important_info_content[1], 3, merge_result,
                                         result["content"],
                                         user_name[0],
                                         current_assistant[0], 1))
                t2 = asyncio.create_task(
                    get_translation_part(memories[-3:], temp_important_info_content[2], 3, merge_result,
                                         result["content"],
                                         user_name[0],
                                         current_assistant[0], 2))
                t3 = asyncio.create_task(
                    get_translation_part(memories[-3:], temp_important_info_content[3], 3, merge_result,
                                         result["content"],
                                         user_name[0],
                ...
                # 获取Translation部分
                t1 = asyncio.create_task(
                    get_translation_part(memories, temp_important_info_content[1], 3, merge_result, result["content"],
                                         user_name[0],
                                         current_assistant[0], 1))
                t2 = asyncio.create_task(
                    get_translation_part(memories, temp_important_info_content[2], 3, merge_result, result["content"],
                                         user_name[0],
                                         current_assistant[0], 2))
                t3 = asyncio.create_task(
                    get_translation_part(memories, temp_important_info_content[3], 3, merge_result, result["content"],
                                         user_name[0],
                                         current_assistant[0], 3))
                if enable_likeability[0] == 0:
                    await t1
